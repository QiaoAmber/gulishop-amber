  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="user in userAddressList" :key="user.id">
        <span class="username" :class="{ selected: user.isDefault === '1' }"
              @click="changeStatus(user,userAddressList)">{{
            user.consignee
          }}</span>
        <p>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ choosedAddress.userAddress }}</span>|
        收货人：<span>{{ choosedAddress.consignee }}</span>|
        <span>{{ choosedAddress.phoneNum }}</span>
      </div>
    </div>
    <div class="sub clearFix">
      <a href="javascript:;" class="subBtn" @click="orderHandler">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  watch: {
    userAddressList: {
      deep: true,
      handler(newV, oldV) {
        this.choosedAddress = newV.filter((item) => item.isDefault === "1")[0];
      },
    }
  },
  methods: {
    ...mapActions(["getTradeInfoAsync"]),
    changeStatus(user, userList) {
      userList.map((item) => (item.isDefault = "0"));
      user.isDefault = "1";
    },
    async orderHandler() {
      ...
      let { tradeNo, detailArrayList: orderDetailList } =
        this.$store.state.trade.tradeInfo;
      let {
        consignee,
      ...
      const result = await this.$API.postSubmitOrder(tradeNo, tradeInfo);
      if (result.code === 200) {
